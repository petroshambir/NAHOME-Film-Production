...
function FilmSection({ section = {}, preview = true }) {
  ...
  return (
    <section className="relative w-full py-16 md:py-28 overflow-hidden bg-[#030303] text-white/90">
      <div className="absolute inset-0 bg-gradient-to-b from-[#030303] via-[#0b0b0b] to-black pointer-events-none" />
      ...
      <div className="relative max-w-7xl mx-auto px-4 md:px-8">
        ...
        {!preview && (
          <div className="mb-8">
            <Link
              to="/"
              className="inline-flex items-center gap-3 text-xs uppercase tracking-[0.3em] text-[#FF4900] hover:underline"
            >
              <span>←</span> Back to Home
            </Link>
          </div>
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-8 mb-12 md:mb-16">
          <div>
            <div className="flex items-center gap-3 mb-5">
              <span className="w-10 h-px bg-[#FF4900]" />
              <span className="text-[9px] md:text-[10px] uppercase tracking-[0.55em] text-[#FF4900] font-bold">
                Film Production
              </span>
            </div>
            <h2 className="text-5xl md:text-7xl font-serif italic font-light text-[#FF4900]">
              {preview ? 'Featured Film' : 'Our Films'}
            </h2>
          </div>

          <p className="max-w-md text-sm md:text-base text-white/60 leading-relaxed md:text-right font-light">
            {section.desc || section.description || 'Cinematic productions created with vision and unforgettable imagery.'}
          </p>
        </div>
          ...
          {displayedMovies.map((movie, movieIndex) => (
            <div key={movie.title} className="grid lg:grid-cols-[1.35fr_0.65fr] gap-5 md:gap-7 items-center">
              ...
              <a
                ...
                rel="noopener noreferrer"
                aria-label={`Watch ${movie.title}`}
                className="group relative overflow-hidden rounded-[1.5rem] md:rounded-[2rem] border border-[#FF4900]/30 bg-black block"
              >
                <div className="relative aspect-video">
                  <ProtectedImage
                    src={movie.poster}
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="w-16 h-16 md:w-24 md:h-24 rounded-full border border-[#FF4900]/40 bg-black/60 backdrop-blur-md flex items-center justify-center transition-all duration-500 group-hover:scale-110 group-hover:bg-[#FF4900] group-hover:border-[#FF4900]">
                      <span className="ml-1 text-xl md:text-3xl text-[#FF4900] group-hover:text-black">▶</span>
                    </div>
                  </div>
                  ...
                  <div className="absolute left-5 md:left-8 right-5 md:right-8 bottom-5 md:bottom-8">
                    ...
                    <h3 className="mt-2 text-xl md:text-3xl font-serif text-[#FF4900]">{movie.title}</h3>
                  </div>
                </div>
              </a>
              ...
              <div className="grid grid-cols-2 gap-3 md:gap-4">
                {movie.behindScenes.slice(0, 4).map((img, index) => (
                  <button
                    ...
                    onClick={() => setActiveImage({ url: img, title: `${movie.title} - BTS ${index + 1}` })}
                    aria-label={`View ${movie.title} BTS ${index + 1} larger`}
                    className="group relative overflow-hidden rounded-xl border border-[#FF4900]/20 bg-black block text-left cursor-pointer"
                  >
                    <ProtectedImage
                      src={img}
                      alt={`${movie.title} BTS ${index + 1}`}
                    ...
                    />
                    <div className="absolute inset-0 bg-black/30 group-hover:bg-black/10 transition-all duration-500" />
                    <span className="absolute bottom-3 left-3 text-[7px] uppercase tracking-[0.25em] text-[#FF4900]">
                      BTS {String(index + 1).padStart(2, '0')}
                    </span>
                  </button>
                ))}
        ...
        <div className="fixed inset-0 z-50 bg-black/95 backdrop-blur-md flex items-center justify-center p-4">
          <div className="relative max-w-4xl w-full max-h-[90vh] flex flex-col items-center">
            <button
              type="button"
              onClick={() => setActiveImage(null)}
              className="absolute -top-12 right-0 text-[#FF4900] text-2xl font-bold bg-[#FF4900]/10 hover:bg-[#FF4900] hover:text-black w-10 h-10 rounded-full flex items-center justify-center transition-colors border border-[#FF4900]/30"
              aria-label="Close modal"
            >
              ×
            </button>
            ...
            <img
              src={activeImage.url}
              alt={activeImage.title}
              className="max-h-[75vh] w-auto object-contain rounded-xl border border-[#FF4900]/30 shadow-2xl"
            />
            <p className="mt-4 text-xs uppercase tracking-[0.3em] text-[#FF4900]">
              {activeImage.title}
            </p>
          </div>
        </div>
