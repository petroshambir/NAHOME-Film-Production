...
const fixImageUrl = (imgObjOrUrl) => {
  ...
  if (typeof imgObjOrUrl === 'string') {
    ...
    if (url.includes('localhost:5000')) {
      url = url.replace('http://localhost:5000', 'https://nahome-film-production.onrender.com');
    }
  ...
  return {
    original: originalUrl.includes('localhost:5000') 
      ? originalUrl.replace('http://localhost:5000', 'https://nahome-film-production.onrender.com') 
      : originalUrl,
    compressed: compressedUrl.includes('localhost:5000') 
      ? compressedUrl.replace('http://localhost:5000', 'https://nahome-film-production.onrender.com') 
      : compressedUrl
  };
};
...
function ClientSelection() {
  ...
  const fetchPortals = async () => {
    try {
      const response = await fetch('https://nahome-film-production.onrender.com/api/client/portals');
      if (response.ok) {
  ...
  const handleVerifyPasscode = async (e) => {
    ...
    try {
      const response = await fetch('https://nahome-film-production.onrender.com/api/client/verify-client-passcode', {
        method: 'POST',
  ...
  const handleSubmitSelection = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://nahome-film-production.onrender.com/api/client/submit-selection/${targetId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
