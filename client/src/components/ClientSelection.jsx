...
//                   onChange={(e) => setEnteredPasscode(e.target.value)}
//                   placeholder="****"
//                   className="bg-zinc-900 border border-[#ff6600]/50 p-3 rounded-xl w-full text-center text-2xl tracking-widest text-zinc-100 font-mono focus:outline-none focus:border-[#ff6600] shadow-inner placeholder-zinc-600"
//                   required
//                 />
//                 {passcodeError && <p className="text-red-400 text-xs font-medium">{passcodeError}</p>}
...
//                     type="submit"
//                     disabled={verifying}
//                     className="bg-[#ff6600] text-white px-4 py-3 rounded-xl text-xs uppercase font-bold tracking-[0.2em] hover:bg-[#e05a00] transition-all w-1/2 disabled:opacity-50 shadow-lg"
//                   >
//                     {verifying ? 'Checking...' : 'Verify & Enter'}
//                   </button>
//                 </div>
...
//                     key={portal._id}
//                     onClick={() => handleSelectClient(portal)}
//                     className="p-4 border border-[#ff6600]/20 bg-zinc-900/50 hover:bg-[#ff6600] hover:text-white transition-all text-left flex flex-col justify-between rounded-xl group shadow-sm"
//                   >
//                     <div>
//                       <span className="text-[10px] uppercase font-bold tracking-widest text-[#ff6600] group-hover:text-white/80">Portal #{portal.portalNumber}</span>
//                       <h3 className="text-base font-serif font-bold text-zinc-100 group-hover:text-white mt-1">{portal.clientName}</h3>
//                     </div>
//                     <span className="text-xs text-zinc-400 group-hover:text-white mt-3 flex items-center gap-1 font-semibold">
//                       Enter Passcode &rarr;
//                     </span>
//                   </button>
//                 ))}
...
//             <div className="w-16 h-16 bg-[#ff6600]/20 text-[#ff6600] border border-[#ff6600]/40 rounded-full flex items-center justify-center mx-auto mb-4 text-2xl shadow-inner">✓</div>
//             <h2 className="text-2xl font-serif mb-2 text-zinc-100">ምርጫኹም ብዕወት ተሰዲዱ ኣሎ!</h2>
//             <div className="w-12 h-[1px] bg-[#ff6600]/40 mx-auto mb-3"></div>
//             <p className="text-xs md:text-sm text-zinc-400 mb-6 font-light">
//               ንሕና ነቲ ዝመረጽኩዎም <b className="text-zinc-200">{selectedImages.length}</b> ስእሊታት ተቐቢልና ኤዲቲንግ ክንጅምር ኢና።
...
//             <button
//               onClick={() => { setProject(null); setSubmitted(false); }}
//               className="bg-[#ff6600] text-white px-6 py-3 text-xs uppercase font-bold tracking-[0.2em] hover:bg-[#e05a00] transition-all rounded-xl shadow-lg"
//             >
//               Back to Portals
//             </button>
//           </div>
...
//                   &larr; Back to Client List
//                 </button>
//                 <span className="text-[10px] uppercase font-bold tracking-widest text-[#ff6600] block">Welcome, {project.clientName}</span>
//                 <h1 className="text-2xl md:text-3xl font-serif text-zinc-100">Portal #{project.portalNumber} - Photo Selection</h1>
//               </div>
             
//               <div className="mt-4 md:mt-0 flex items-center gap-4">
//                 <div className="bg-zinc-900 border border-[#ff6600]/30 text-zinc-200 px-6 py-3 rounded-xl shadow-inner text-sm font-medium">
//                   Selected Images: <span className="text-[#ff6600] font-bold text-lg">{selectedImages.length}</span>
//                 </div>
//                 <button
//                   onClick={handleSubmitSelection}
//                   disabled={loading || selectedImages.length === 0}
//                   className="bg-[#ff6600] text-white px-6 py-3 text-xs uppercase font-bold tracking-[0.2em] hover:bg-[#e05a00] transition-all disabled:opacity-50 shadow-lg rounded-xl flex items-center gap-2"
//                 >
//                   {loading ? 'Sending...' : 'Send to Studio'}
//                 </button>
//               </div>
...
//                       onClick={() => handleCheckboxChange(imgObj)}
//                       className={`relative group overflow-hidden border-2 rounded-xl transition-all aspect-square bg-zinc-900 cursor-pointer ${
//                         isSelected ? 'border-[#ff6600] shadow-2xl scale-[0.98]' : 'border-zinc-800 hover:border-zinc-600'
//                       }`}
//                     >
//                       <div className="w-full h-full">
//                         <ProtectedImage
...
//                           <span className="bg-[#ff6600] text-white text-[10px] uppercase font-bold px-2.5 py-1 tracking-widest shadow-md rounded-md">
//                             Selected
//                           </span>
//                         </div>
//                       )}
      ...
      <div className="flex-grow flex flex-col items-center justify-center px-4 py-24">
        {selectedPortalForPasscode && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-zinc-950 p-8 rounded-2xl shadow-2xl max-w-md w-full border-2 border-[#ff6600]/40 text-center relative">
              ...
              <h3 className="text-2xl font-serif font-bold text-[#2563eb] mb-2">Enter Portal Passcode</h3>
              <div className="w-12 h-[1px] bg-[#ff6600]/40 mx-auto mb-3"></div>
              <p className="text-xs md:text-sm text-zinc-400 mb-6 font-light">
                ናብ <b className="text-[#2563eb]">{selectedPortalForPasscode.clientName}</b> ፖርታል ንምእታው በጃኹም እቲ ካብ ስቱድዮ ዝተዋህበኩም 4-ቁጽሪ ኮድ ኣእትዉ።
              </p>
        ...
        {!project ? (
          <div className="bg-zinc-950 p-8 md:p-12 shadow-2xl border-2 border-[#ff6600]/30 rounded-2xl max-w-xl w-full text-center">
            ...
            <h2 className="text-2xl md:text-3xl font-serif mb-2 text-[#2563eb]">Client Photo Selection Portals</h2>
            <div className="w-12 h-[1px] bg-[#ff6600]/40 mx-auto mb-3"></div>
            <p className="text-xs md:text-sm text-zinc-400 mb-6 font-light">በጃኹም ንምርጫ ስእሊታት ናይቲ ስቱድዮ ሽምኩም ጠውቑ።</p>
            ...
            {fetchingPortals ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-96 overflow-y-auto p-1 scrollbar-thin">
                {portals.map((portal) => (
                  ...
                  >
                    <div>
                      <span className="text-[10px] uppercase font-bold tracking-widest text-[#ff6600] group-hover:text-[#2563eb]/80">Portal #{portal.portalNumber}</span>
                      <h3 className="text-base font-serif font-bold text-[#2563eb] group-hover:text-[#2563eb] mt-1">{portal.clientName}</h3>
                    </div>
          ...
          <div className="bg-zinc-950 p-10 shadow-2xl border-2 border-[#ff6600]/40 rounded-2xl max-w-md w-full text-center">
            <div className="w-16 h-16 bg-[#ff6600]/20 text-[#ff6600] border border-[#ff6600]/40 rounded-full flex items-center justify-center mx-auto mb-4 text-2xl shadow-inner">✓</div>
            <h2 className="text-2xl font-serif mb-2 text-[#2563eb]">ምርጫኹም ብዕወት ተሰዲዱ ኣሎ!</h2>
            <div className="w-12 h-[1px] bg-[#ff6600]/40 mx-auto mb-3"></div>
            <p className="text-xs md:text-sm text-zinc-400 mb-6 font-light">
              ንሕና ነቲ ዝመረጽኩዎም <b className="text-[#2563eb]">{selectedImages.length}</b> ስእሊታት ተቐቢልና ኤዲቲንግ ክንጅምር ኢና።
            </p>
            <button
              onClick={() => { setProject(null); setSubmitted(false); }}
          ...
          <div className="max-w-7xl w-full mx-auto px-4">
            <div className="flex flex-col md:flex-row justify-between items-center mb-8 border-b border-zinc-800 pb-6 sticky top-20 bg-[#050505]/95 backdrop-blur-md z-10 py-4">
              <div>
                ...
                <span className="text-[10px] uppercase font-bold tracking-widest text-[#ff6600] block">Welcome, {project.clientName}</span>
                <h1 className="text-2xl md:text-3xl font-serif text-[#2563eb]">Portal #{project.portalNumber} - Photo Selection</h1>
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div 
                        ...
                        className="absolute bottom-2 left-2 z-10 bg-black/60 hover:bg-black text-[9px] uppercase tracking-wider text-[#2563eb] px-2 py-1 rounded backdrop-blur-md cursor-pointer border border-white/10"
                      >
                        🔍 Zoom
                      </div>
              ...
              <div className="relative w-full h-full flex flex-col items-center justify-center p-2 select-none" onContextMenu={(e) => e.preventDefault()}>
                ...
                <div className="mt-2 mb-4 z-50">
                  <button
                    ...
                    className={`px-6 py-2.5 rounded-xl text-xs uppercase font-bold tracking-widest transition-all shadow-lg ${
                      isCurrentSelected 
                        ? 'bg-red-500/80 text-[#2563eb] hover:bg-red-600' 
                        : 'bg-[#ff6600] text-[#2563eb] hover:bg-[#e05a00]'
                    }`}
                  >
                    {isCurrentSelected ? 'Remove Selection (-)' : 'Select This Photo (+)'}
                  </button>
